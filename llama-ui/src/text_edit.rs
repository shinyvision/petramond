...
impl<C: TextClipboard + ?Sized> TextClipboard for &mut C {
    fn get_text(&mut self) -> Option<String> {
        (**self).get_text()
    }
    fn set_text(&mut self, text: &str) -> bool {
...
impl TextInput {
    ...
    pub fn move_home(&mut self, extend_selection: bool, visible_chars: usize, now: f64) {
        if extend_selection {
            self.selection_anchor.get_or_insert(self.cursor);
        } else {
            self.selection_anchor = None;
    ...
    pub fn move_end(&mut self, extend_selection: bool, visible_chars: usize, now: f64) {
        if extend_selection {
            self.selection_anchor.get_or_insert(self.cursor);
        } else {
            self.selection_anchor = None;
...
mod tests {
    ...
    #[test]
    fn with_text_preloads_and_ends_cursor() {
