...
pub fn body_separation_from_body(
    ...
    let other_pos = WorldPos::new(other.x, other.y0, other.z);
    let other_size = MobSize {
        half_width: other.hw,
        height: (other.y1 - other.y0) as f32,
        half_length: None,
    };
...
fn body_box_is_supported_by(
    ...
    let gap = (pos.y - candidate.max[1]) as f32;
    if !(-PEER_SUPPORT_VERTICAL_EPS..=PEER_SUPPORT_VERTICAL_EPS).contains(&gap) {
        return false;
    }
    let eps = f64::from(PEER_SUPPORT_HORIZONTAL_EPS);
    body_boxes(pos, yaw, size).any(|(min, max)| {
        max[0] > candidate.min[0] + eps
            && min[0] < candidate.max[0] - eps
            && max[2] > candidate.min[2] + eps
            && min[2] < candidate.max[2] - eps
    })
}
...
fn compound_push_bounds_overlap(
    ...
    if a_pos.y + f64::from(a_size.height) <= b_pos.y
        || b_pos.y + f64::from(b_size.height) <= a_pos.y
    {
        return false;
    }
    let dx = (a_pos.x - b_pos.x) as f32;
    let dz = (a_pos.z - b_pos.z) as f32;
    let a_reach = a_size
        .half_length
...
fn aabb_cells_known<K>(min: [f64; 3], max: [f64; 3], known: &K) -> bool
where
    K: Fn(i32, i32, i32) -> bool,
{
    let hi = max.map(|edge| {
    ...
    F: Fn(i32, i32, i32) -> &'static [petramond_world::block::Aabb],
{
    segment_offsets(size).any(|offset| {
        let (min_x, max_x) = arc_component_bounds(from, to, std::f32::consts::FRAC_PI_2, |yaw| {
            segment_offset(yaw, offset).x
        });
        let (min_z, max_z) =
            arc_component_bounds(from, to, 0.0, |yaw| segment_offset(yaw, offset).z);
        let hw = f64::from(size.half_width);
        let min = [
            pos.x + f64::from(min_x) - hw,
            pos.y,
            pos.z + f64::from(min_z) - hw,
        ];
        let max = [
            pos.x + f64::from(max_x) + hw,
            pos.y + f64::from(size.height),
            pos.z + f64::from(max_z) + hw,
        ];
        petramond_world::collision::aabb_hits_cells(min, max, boxes_fn)
...
pub fn closest_body_ray_hit<K: Ord>(
    ...
    dir: Vec3,
    max_dist: f32,
    bodies: impl IntoIterator<Item = (K, WorldPos, f32, MobSize)>,
) -> Option<(K, f32)> {
    let mut best = None;
    for (key, pos, yaw, size) in bodies {
        ...
        let hit = body_boxes(pos, yaw, size)
            .filter_map(|(min, max)| {
                ...
                crate::player::ray_vs_aabb(Vec3::ZERO, dir, rel(min), rel(max))
            })
