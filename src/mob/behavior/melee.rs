...
//! body half-width), the mob is roughly facing them, and the per-node cooldown has
//! elapsed, the node emits an [`AttackIntent`]. It never touches the player itself:
//! the intent flows instance → manager → `Game`, where the damage runs through the
//! `player_damage_pre` pipeline (a cancel drops the knockback too). Deliberately
//! simple targeting — range + rough facing, no line-of-sight ray.
//!
...
impl AiBehavior for MeleeAttackAi {
    fn tick(&mut self, ctx: &mut AiCtx) -> BehaviorOutput {
        ...
        if gap > self.reach || !facing_player(ctx.yaw, ctx.pos, ctx.player_pos) {
            return BehaviorOutput::default();
        }
