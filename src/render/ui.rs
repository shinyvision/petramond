...
pub fn craft_slot_rect(kind: CraftKind, i: usize, screen: (u32, u32), scale: f32) -> Option<SlotRect> {
    let cols = kind.cols();
    if i >= cols * cols {
        return None;
    }
...
pub fn craft_slot_at_cursor(
    ...
    let scale = gui_scale(screen);
    let (px, py) = cursor_px;
    let inside = |r: SlotRect| px >= r.x && px < r.x + r.w && py >= r.y && py < r.y + r.h;
    ...
    inside(craft_result_rect(kind, screen, scale)).then_some(CraftHit::Result)
}
...
pub fn build_ui(ui: &UiSnapshot, build: &mut UiBuild) {
    ...
    if ui.open {
        ...
        let (ox, oy) = panel_origin(screen, scale);
        let panel_sprite = match ui.panel {
    ...
    // --- Crafting grid input cells + result preview (open panels only). ---
    if ui.open {
        let kind = ui.panel;
        for i in 0..kind.cols() * kind.cols() {
...
mod tests {
    ...
    fn open_build_draws_craft_cells_and_result() {
        ...
        for i in [0usize, 4] {
            ...
            assert_eq!(slot_at_cursor(s.screen, true, c), None, "craft cell {i} not an inv slot");
            assert_eq!(craft_slot_at_cursor(CraftKind::Table, s.screen, c), Some(CraftHit::Input(i)));
        }
        let rr = craft_result_rect(CraftKind::Table, s.screen, scale);
        let rc = (rr.x + rr.w * 0.5, rr.y + rr.h * 0.5);
        assert_eq!(craft_slot_at_cursor(CraftKind::Table, s.screen, rc), Some(CraftHit::Result));
    }
