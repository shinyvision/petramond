...
pub(in crate::render) struct GpuTimer {
...
impl GpuTimer {
    ...
    pub(in crate::render) fn wanted() -> bool {
        std::env::var("PETRAMOND_GPU_TIMING").is_ok_and(|v| v != "0")
    }

    pub(in crate::render) fn new(device: &wgpu::Device, queue: &wgpu::Queue) -> Option<Self> {
    ...
    /// thing encoded.
    pub(in crate::render) fn finish_frame(&self, enc: &mut wgpu::CommandEncoder) {
    ...
    pub(in crate::render) fn after_submit(&self, device: &wgpu::Device) {
        let mut st = self.state.borrow_mut();
        if st.used == 0 {
            st.labels.clear();
            return;
    ...
    pub(in crate::render) fn cpu_stage(&self, label: &'static str, ns: f64) {
        let mut st = self.state.borrow_mut();
    ...
    pub(in crate::render) fn report(&self) -> Vec<(&'static str, f64, u32)> {
        self.state.borrow().totals.clone()
    }

    pub(in crate::render) fn report_cpu(&self) -> Vec<(&'static str, f64, u32)> {
        self.state.borrow().cpu.clone()
    }

    pub(in crate::render) fn reset(&self) {
        let mut st = self.state.borrow_mut();
        st.totals.clear();
        st.cpu.clear();
    }
