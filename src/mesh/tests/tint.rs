...
fn a_stack_dyed_per_layer_draws_each_layer_in_its_own_colour() {
    ...
    section.set_slab_state(
        ...
        1,
        SlabState {
            split: crate::block_state::SlabSplit::Y,
    ...
    section.cell_kv_set(
        ...
        1,
        1,
        crate::block::part_kv_key(TINT_KV_KEY, 1),
        top.to_vec(),
    );
    ...
    let colours = |above: bool| -> Vec<[f32; 3]> {
        verts
            ...
            .map(|v| crate::mesh::unpack_tint(v.tint))
            .collect()
    };
...
fn the_cube_fast_path_follows_whether_the_layers_agree_on_colour() {
    let stack = |tints: &[(crate::block::CellPart, [u8; 3])]| {
        let mut section = section_with(&[((1, 1, 1), Block::WoolSlab)]);
        section.set_slab_state(
            1,
            1,
        ...
        for &(part, rgb) in tints {
            section.cell_kv_set(
                ...
                1,
                1,
                crate::block::part_kv_key(TINT_KV_KEY, part),
                rgb.to_vec(),
            );
        }
