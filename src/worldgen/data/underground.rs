...
fn parse_layers(texts: &[&str]) -> Result<UndergroundBiomes, String> {
...
fn convert_faces(f: RawFaces, base: u8, name: &'static str) -> Result<ConvertedFaces, String> {
    ...
    let mut one = |raw: Option<RawFace>, which: usize, label: &str| -> Result<FaceLining, String> {
        ...
        names[which] = crate::registry::names().blocks.name(block).unwrap_or("?");
        Ok(FaceLining {
            block,
            weight: raw.weight,
        })
...
/// layers, leaked so it can drive a [`CaveField`](crate::worldgen::noise::height::CaveField)
