...
fn main() {
    ...
    for &(cx,cz) in &coords {
        ...
        let mut mh = 0u16;
        for &h in c.heightmap.iter() { if h > mh { mh = h; } }
        sum_maxh += mh as u64;
    ...
    println!("chunks={} avg_maxh={:.1} min={} max={}", n, sum_maxh as f64/n as f64, min_maxh, max_maxh);
    println!("full voxels scanned   = {}", sum_volume_scanned_full);
    println!("bounded voxels scanned= {} ({:.1}% of full)", sum_volume_scanned_bounded,
