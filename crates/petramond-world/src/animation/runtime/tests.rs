...
fn a_crossfade_blends_continuously_and_reentering_a_fading_state_never_pops() {
    ...
    lib.insert(
        "walk",
        clip(&m, 1.0, true, &[("leftArm", 0.0, Vec3::X * 60.0)]),
    );
    let mut a = animator(
        &m,
        lib,
    ...
    assert!(
        (fading[4] - 15.0).abs() < 1e-3,
        "a quarter through a linear fade: {fading:?}"
    );
    ...
    for x in &back {
        assert!(
            (x - prev).abs() < 4.0,
            "no pop turning back mid-fade: {prev} -> {x}"
        );
        prev = *x;
    }
...
fn a_synced_blend_shares_one_phase_and_fires_its_heaviest_gaits_markers_however_frames_are_cut() {
    ...
    assert!(
        (x[x.len() - 1] - 40.0).abs() < 0.5,
        "both gaits 40% through a 0.75 s cycle: {}",
        x[x.len() - 1]
    );
...
fn a_montage_fades_in_over_its_mask_and_finishes_fading_as_its_clip_ends() {
    let m = rig();
    let (left, right) = (
        m.bone_named("leftArm").unwrap(),
        m.bone_named("rightArm").unwrap(),
    );
    let both = |deg: f32| {
        [
            ("leftArm", 0.0, Vec3::X * deg),
            ("rightArm", 0.0, Vec3::X * deg),
        ]
    };
    let mut lib = ClipLibrary::new();
    ...
    for _ in 0..60 {
        ...
        assert!(
            (r - 20.0).abs() < 1e-4,
            "right arm: base plus half the additive, never the swing ({r})"
        );
    }
    ...
    assert!(
        (lefts[49] - 10.0).abs() < 1.0,
        "the fade lands with the clip's end: {}",
        lefts[49]
    );
...
fn an_interrupting_montage_covers_the_one_it_cut_off_without_dipping_to_the_ground() {
    ...
    lib.insert(
        "swing",
        clip(&m, 0.5, false, &[("leftArm", 0.0, Vec3::X * 90.0)]),
    );
    let mut a = animator(
        &m,
        lib,
...
fn a_looping_segment_finishes_its_cycle_before_chaining_to_the_end() {
    ...
    let arm = |len: f32, looping: bool, from: f32, to: f32| {
        clip(
            ...
            looping,
            &[
                ("leftArm", 0.0, Vec3::X * from),
                ("leftArm", len, Vec3::X * to),
            ],
        )
    };
    ...
    advance(&mut a, 85);
    assert_eq!(
        playing(&a, "action").as_deref(),
        Some("loop"),
        "third cycle, from 0.8 s"
    );
...
fn picks_cycle_and_reset_after_a_pause_and_random_picks_never_repeat() {
    ...
    for name in ["a", "b", "c"] {
        assert!(
            rolls.iter().any(|r| r == name),
            "{name} never picked: {rolls:?}"
        );
    }
}
...
fn a_mirrored_montage_plays_on_the_partner_bones_reflected() {
    ...
    let mut raise = clip(
        ...
        1.0,
        false,
        &[("leftArm", 0.0, Vec3::new(30.0, 20.0, 10.0))],
    );
    raise.set_track(
        left,
        Channel::Position,
...
fn hitstop_freezes_a_slots_clip_then_lets_it_run_on() {
    ...
    a.fire_named("hit");
    assert!(
        (run(&mut a, 0.1, DT, arm)[9] - 30.0).abs() < 1e-3,
        "frozen on the hit"
    );
    let after = run(&mut a, 0.1, DT, arm)[9];
    assert!(
        (after - 40.0).abs() <= 1.01,
        "runs on from where it froze: {after}"
    );
}
...
fn an_inertial_montage_starts_from_the_old_pose_and_settles_without_overshoot() {
    ...
    lib.insert(
        "hold",
        clip(&m, 2.0, false, &[("leftArm", 0.0, Vec3::X * 90.0)]),
    );
    let mut a = animator(
        &m,
        lib,
    ...
    assert!(
        x[24] > 87.0,
        "within 5% soon after its settle time: {}",
        x[24]
    );
}
...
fn a_ground_pose_shows_through_everything_the_graph_does_not_own() {
    ...
    lib.insert(
        "raise",
        clip(&m, 1.0, false, &[("leftArm", 0.0, Vec3::X * 90.0)]),
    );
    let mut a = animator(
        &m,
        lib,
    ...
    assert_eq!(
        a.pose().rotation(left),
        Vec3::X * 10.0,
        "nothing playing: the ground"
    );
    a.fire_named("use");
    a.update_over(DT, &walk);
    assert_eq!(
        a.pose().rotation(left),
        Vec3::X * 90.0,
        "the montage owns its masked channel"
    );
    assert_eq!(
        a.pose().rotation(right),
        Vec3::new(10.0, 5.0, 0.0),
        "additive layers add onto the ground"
    );
...
fn a_scrubbed_montage_samples_where_its_caller_puts_it_and_fires_what_it_passes() {
    ...
    let mut a = animator(
        &m,
        lib,
        r#"{ "slots": ["action"], "layers": [{ "slot": "action" }] }"#,
    );
    ...
    for (seconds, want, fired) in [
        ...
        a.update(DT);
        assert!(
            (a.pose().rotation(arm).x - want).abs() < 1e-3,
            "at {seconds}: {}",
            a.pose().rotation(arm).x
        );
...
fn a_rule_the_slot_refuses_keeps_its_cooldown_and_its_pick() {
    ...
    assert_eq!(
        playing(&a, "other").as_deref(),
        Some("flinch"),
        "a refused rule is no match: the next fires"
    );
    a.stop(slot, 0.0);
    a.update(DT);
    a.fire_named("swing");
    ...
    assert_eq!(
        playing(&a, "action").as_deref(),
        Some("a"),
        "neither the cooldown nor the combo moved"
    );
}
...
fn an_event_fired_twice_before_an_update_runs_its_rules_once() {
    ...
    assert_eq!(
        playing(&a, "action").as_deref(),
        Some("b"),
        "the double fire advanced the combo once"
    );
}
...
fn a_seek_moves_only_the_play_it_names_and_a_cut_off_play_reads_displaced() {
    ...
    assert!(
        (after - before - DT).abs() < 1e-4,
        "the swing ran on at its own rate: {before} -> {after}"
    );
}
...
fn a_play_that_ends_reads_finished_and_stopping_one_play_spares_the_other() {
    let m = rig();
    let mut lib = ClipLibrary::new();
    let short = lib.insert(
        "short",
        clip(&m, 0.05, false, &[("leftArm", 0.0, Vec3::ZERO)]),
    );
    let long = lib.insert(
        "long",
        clip(&m, 1.0, false, &[("leftArm", 0.0, Vec3::X * 5.0)]),
    );
    let mut a = animator(
        &m,
        lib,
...
fn a_gate_stands_down_the_rules_it_names_and_no_other() {
    ...
    assert_eq!(
        playing(&a, "feedback").as_deref(),
        Some("flash"),
        "the other slot's rule plays"
    );
...
fn a_mapped_marker_fires_its_event_next_update_and_a_mirrored_clip_swaps_its_side() {
    ...
    a.update(DT);
    assert_eq!(
        playing(&a, "hit").as_deref(),
        Some("right"),
        "a mirrored left foot is the right"
    );
}
...
fn advancing_unposed_runs_rules_and_montages_and_leaves_the_pose() {
    ...
    a.update(DT);
    assert!(
        (a.pose().rotation(arm).x - 31.0).abs() < 1e-2,
        "{}",
        a.pose().rotation(arm).x
    );
}
