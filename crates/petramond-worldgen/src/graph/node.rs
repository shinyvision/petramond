...
impl ScalarGraph {
    #[cfg(test)]
    pub fn has_channel(&self, channel: impl AsRef<str>) -> bool {
    ...
    pub fn channel_node(&self, channel: impl AsRef<str>) -> Option<NodeId> {
        self.outputs.get(channel.as_ref()).copied()
    }

    pub fn node_depends_on_y(&self, node: NodeId) -> bool {
        self.assert_existing_node(node, "Y-dependency query root");
        self.y_dependencies[node.index]
    }

    #[cfg(test)]
    pub fn channel_depends_on_y(&self, channel: impl AsRef<str>) -> Option<bool> {
    ...
    pub fn evaluation_cache(&self) -> GraphEvaluationCache {
        GraphEvaluationCache::new(self)
    }

    pub fn evaluate_node(&self, node: NodeId, point: SamplePoint) -> f64 {
        self.assert_existing_node(node, "evaluation root");
        self.evaluate_node_uncached(node, point)
    }

    pub fn evaluate_node_cached(
        &self,
        node: NodeId,
        point: SamplePoint,
...
pub fn ridge_fold_value(variance: f64) -> f64 {
