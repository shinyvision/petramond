...
pub(super) fn section_geometry(
    ...
    let part_tint = |cell: usize, part: petramond_world::block::CellPart| -> Option<[f32; 3]> {
    ...
    let slab_at = |wx: i32, wy: i32, wz: i32| -> Option<SlabState> {
        ...
        petramond_world::slab::is_slab(block).then(|| {
    ...
    for ly in 0..SECTION_SIZE {
        ...
        for lz in 0..SECTION_SIZE {
            ...
            while row != 0 {
                ...
                row &= row - 1;
                let resident = Block::from_id(section.block_raw(lx, ly, lz));
                for block in std::iter::once(resident).chain(resident.contained_fluid()) {
                    ...
                    if class & TORCH != 0 {
                        ...
                        let sky6 =
                            ((cell_sky * 63 + SKY_FULL as u32 / 2) / SKY_FULL as u32).min(63);
...
impl<B, S> petramond_world::block::ShapeNeighborhood for MeshNeighborhood<'_, B, S>
where
    B: Fn(i32, i32, i32) -> u16,
    S: Fn(i32, i32, i32) -> petramond_world::block::ShapeState,
{
    fn block(&self, pos: IVec3) -> Block {
        Block::from_id((self.block)(pos.x, pos.y, pos.z))
    }

    fn shape_state(&self, pos: IVec3) -> petramond_world::block::ShapeState {
