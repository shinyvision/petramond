...
use petramond::sound_registry::defs as sound_defs;
        ...
        let buffers = sound_defs()
            ...
            .map(|def| {
                def.variants
                    .iter()
                    .filter_map(|&rel| {
                        let Some((bytes, _)) = petramond::assets::read_bytes(rel) else {
...
mod tests {
    ...
    fn wood_punch_variant_decodes_to_pcm() {
        ...
        let bytes = petramond::assets::read_bytes(rel).expect("clip file exists").0;
